...
func TestE2E_HappyPath(t *testing.T) {
	...
	body, err := h.client.Save(context.Background(), vrcclient.SaveParams{Data: &savedata.Data{Score: 1234}, JWT: jwt, DisplayName: "alice"})
	if err != nil {
		t.Fatalf("Save: %v", err)
	}
	if body != "success" {
...
func TestE2E_RenameInvalidatesOldEntry(t *testing.T) {
	...
	_, _ = h.client.Save(context.Background(), vrcclient.SaveParams{Data: &savedata.Data{Score: 100}, JWT: jwt1, DisplayName: "alice"})

	jwt2 := h.register("discord-1", "alice2")
	_, _ = h.client.Save(context.Background(), vrcclient.SaveParams{Data: &savedata.Data{Score: 999}, JWT: jwt2, DisplayName: "alice2"})
...
func TestE2E_BanHidesUserFromRanking(t *testing.T) {
	h := newHarness(t)
	jwt := h.register("discord-1", "alice")
	_, _ = h.client.Save(context.Background(), vrcclient.SaveParams{Data: &savedata.Data{Score: 1234}, JWT: jwt, DisplayName: "alice"})
...
func TestE2E_SaveWithoutJWT_Rejected(t *testing.T) {
	...
	_, err := h.client.Save(context.Background(), vrcclient.SaveParams{Data: &savedata.Data{Score: 9999}, DisplayName: "alice"})
	if err == nil {
