...
HMAC署名に加えてDiscord OAuth連携のJWTを併用し、データを保護する。
...
| ユーザー名の状態 | 必要な認証                                                             |
| ---------------- | ---------------------------------------------------------------------- |
| Discord未連携    | HMACのみ (従来どおりの仕様を維持)                                      |
| Discord連携済み  | HMAC + JWT (クレームのユーザー名一致 / 現行JTI / ブラックリスト未登録) |

### トークン発行 = 自分のデータを守るアクション

ワールドアセットからHMAC鍵が漏洩しても、被害はDiscord未連携のユーザー名に限定される。
登録済みユーザー名はJWT検証が必須であるため、攻撃者がHMAC鍵だけでは上書きできない。
...
- `display_name` が **未登録** (DB に存在しない) なら JWT なしで通る。HMAC のみ検証
...
これは、Udon に埋め込まれた HMAC 秘密鍵が抽出可能であることを前提とした多層防御:
正規ユーザーは一度登録すれば JWT で自分の DisplayName をロックでき、第三者が
HMAC 鍵だけでは上書きできない (詳しくは「既知の制約」参照)。
...
- `sig` = `HMAC-SHA256(HMAC_SAVE_SECRET, <data raw bytes> ‖ 0x00 ‖ <display_name bytes>)` → lowercase hex
...
- リクエスト sig = `HMAC-SHA256(HMAC_LOAD_SECRET, <display_name bytes>)` → lowercase hex
- セーブなしの場合は `404`
- **レスポンス (200)**: JSON
...
レスポンス `sig` = `HMAC-SHA256(HMAC_LOAD_SECRET, <data の raw JSON bytes>)` → lowercase hex
